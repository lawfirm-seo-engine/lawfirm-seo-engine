/**
 * mdxTemplate.ts
 * create-case.js의 핵심 로직을 TypeScript로 포팅.
 * 서버 API 라우트에서 MDX 파일 내용을 생성할 때 사용합니다.
 */
...
export type CaseTypeKey =
  | "쇼핑몰" | "여행사" | "영화사" | "부업"
  | "주식리딩방" | "코인" | "라이브방송" | "로맨스스캠" | "기타"

export function detectCaseType(text: string): CaseTypeKey {
...
const TYPE_DETAILS: Record<CaseTypeKey, TypeDetail> = {
  ...
  "여행사": {
    label: "여행사·팀미션", caseType: "여행사 팀미션 사기",
    ...
    caseA: "여행 패키지 예약을 대행해 주겠다는 안내를 받고 예약금을 입금했지만, 이후 비자 수수료나 추가 좌석 비용 명목으로 반복 입금을 요구받은 유형입니다.",
    caseB: "여행사 팀미션을 통해 예약 건당 수익이 발생한다는 안내를 받고 투자금을 넣었으나, 출금 시 위약금·세금 납부를 요구받은 유형입니다.",
  ...
  "영화사": {
    label: "영화사·팀미션", caseType: "영화사 팀미션 사기",
    ...
    caseA: "영화사 직원을 자칭한 담당자로부터 영상 구매 팀미션 참여 안내를 받은 뒤, 영상을 구매하면서 포인트가 적립되는 구조로 진행됩니다.",
    caseB: "영화 제작 투자 명목으로 소액부터 큰 금액까지 단계적으로 입금을 유도하는 유형입니다.",
    caseC: "미션을 완료하고 출금을 신청하자 계정 등급이 부족하다거나 세금을 먼저 납부해야 한다는 안내를 받은 유형입니다.",
    stopMessage: "출금 조건이나 세금 납부를 요구받고 있다면 추가 입금을 즉시 중단하세요.",
    descSuffix: "영화사 사칭·팀미션 사기 피해 신고. 영상 구매 미션·수익 출금 거부 피해 사례 및 피해금 회복 방법 안내.",
  },
  "부업": {
    label: "부업·팀미션", caseType: "부업 팀미션 사기",
    ...
    caseA: "영상 시청이나 리뷰 작성으로 포인트를 적립하는 방식으로 시작하지만, 환전 신청 시 보증금이나 등급 업그레이드 비용을 요구받는 유형입니다.",
  ...
  "주식리딩방": {
    label: "주식리딩방·투자", caseType: "주식리딩방 투자 사기",
    ...
    caseA: "텔레그램이나 카카오 오픈채팅에서 전문가를 자칭한 담당자로부터 종목 추천을 받아 투자했고, 수익이 발생했다는 화면을 보여주며 추가 증거금을 요구받은 유형입니다.",
    caseB: "소액 투자로 수익을 확인한 뒤 더 큰 금액을 넣었으나, 출금 시 세금 선납 또는 수익금 공증비를 요구받은 유형입니다.",
  ...
  "코인": {
    label: "코인·가상자산", caseType: "코인 가상자산 사기",
    ...
    caseA: "텔레그램 코인 채널에서 스테이킹 수익을 안내받고 투자했으나, 스테이킹 기간 종료 후 출금 시 세금·수수료 명목으로 추가 납부를 요구받은 유형입니다.",
    ...
    stopMessage: "출금 수수료나 지갑 잠금 해제 비용 요구가 있다면 즉시 추가 납부를 중단하세요.",
    descSuffix: "코인거래소 사칭·가상자산 투자 사기 피해 신고. 스테이킹 수익 출금 불가·세금 명목 피해 사례 및 대응 안내.",
  },
  "라이브방송": {
    label: "라이브방송·팀미션", caseType: "라이브방송 팀미션 사기",
  ...
  "로맨스스캠": {
    ...
    caseA: "외국인 또는 해외 교포를 자칭하는 상대방과 SNS에서 연락을 주고받다가, 본인이 운영 중인 투자 플랫폼을 소개받고 투자금을 입금한 유형입니다.",
    caseB: "공항이나 세관에 물건이 묶였다며 통관 비용 대납을 요청받은 유형입니다.",
    caseC: "갑작스러운 의료비나 비상 상황을 이유로 송금을 요청받은 유형입니다.",
  ...
  "기타": {
    ...
    caseA: "처음에는 소액 결제로 신뢰를 쌓은 뒤 점차 큰 금액의 입금을 유도하는 유형입니다.",
...
type TableRow = { stage: string; flow: string; method: string; analysis: string }

const TYPE_TABLES: Record<CaseTypeKey, TableRow[]> = {
  "주식리딩방": [
  ...
  "여행사": [
    ...
    { stage: "5단계", flow: "환불 거부 및 위약금 통보",  method: "취소 위약금 발생, 환불 불가 통보, 추가 오류 해결비 요구",                                    analysis: "기납입금 회수를 방해하며 반복 입금을 유도" },
...
const AFTER_HARD_STOP  = new Set(["피해","허위","사례","어플","앱","안내","복구","신고","방법","경고","주의"])
const AFTER_BRAND_STOP = new Set(["사칭","리딩방","해외선물","지수거래","자동매매","투자","코인","비트코인","국내주식"])
...
function extractSeoKeyword(caseName: string): string {
  ...
  for (const token of afterPart.split(/\s+/).filter(Boolean)) {
    ...
    if (AFTER_BRAND_STOP.has(token)) { if (taken.length === 0) taken.push(token); break }
...
const SUFFIX_MAP: Record<CaseTypeKey, { titleSuffix: string; h1Suffix: string; typeWord: string }> = {
...
function buildTypedTitle(caseName: string, kind: "meta" | "h1"): string {
  ...
  const ct        = detectCaseType(caseName)
  const map       = SUFFIX_MAP[ct] || SUFFIX_MAP["기타"]
...
function buildTypeTable(caseName: string, slug: string): string {
  const ct   = detectCaseType(`${slug} ${caseName}`)
  const rows = TYPE_TABLES[ct] || TYPE_TABLES["기타"]
...
function buildRoleBody(caseName: string, slug: string): string {
  const cleanCaseName   = caseName.trim()
  const caseDisplayName = cleanCaseName.includes("사칭") ? cleanCaseName : `${cleanCaseName} (사칭)`
  const name      = caseDisplayName
  const plainName = cleanCaseName
  const td        = TYPE_DETAILS[detectCaseType(`${slug} ${cleanCaseName}`)] || TYPE_DETAILS["기타"]
...
특히 변호사의 후불제 수임은 법적으로 허용되지 않는 방식이므로, 후불제를 언급하는 경우 또 다른 사기 가능성을 반드시 의심해야 합니다.`
  ...
  const table = buildTypeTable(cleanCaseName, slug)
...
export type GenerateMdxOptions = {
  caseName:           string
  groupId?:           string
...
export type GeneratedMdx = {
  slug:        string
  frontmatter: string
  body:        string
  full:        string   // frontmatter + body 합친 완성본
}

export function generateMdx(opts: GenerateMdxOptions): GeneratedMdx {
  const { caseName, groupId = "", representativeSlug = "" } = opts
  const cleanCaseName   = caseName.trim().replace(/\s+/g, " ")
  const caseDisplayName = cleanCaseName.includes("사칭") ? cleanCaseName : `${cleanCaseName} (사칭)`

  const slug = cleanCaseName.toLowerCase().replace(/\s+/g, "-").replace(/[^\w가-힣-]/g, "")
  const today = opts.today || new Date().toISOString().slice(0, 10)
  const createdAt = new Date().toISOString()

  const ct          = detectCaseType(`${slug} ${cleanCaseName}`)
  const td          = TYPE_DETAILS[ct] || TYPE_DETAILS["기타"]
  const seoTitle    = buildTypedTitle(cleanCaseName, "meta")
  const h1Title     = buildTypedTitle(cleanCaseName, "h1")
  const description = `${cleanCaseName} ${td.descSuffix}`
  const primaryKeyword = cleanCaseName
  const aliases = [cleanCaseName, slug.replace(/-/g, " ")].filter((v, i, a) => a.indexOf(v) === i).join(", ")

  const repLine = representativeSlug && representativeSlug !== slug
    ? `representativeSlug: "${representativeSlug}"`
    : ""

  const frontmatter = `title: "${seoTitle}"
...
aliases: "${aliases}"
caseType: "${td.caseType}"${repLine ? `\n${repLine}` : ""}`

  const representativeBlock = representativeSlug && representativeSlug !== slug
    ? `\n## 관련 대표 사건 안내\n\n해당 사건은 아래 대표 사건과 동일 유형입니다.\n\n👉 /cases/${representativeSlug}\n\n`
    : ""

  const imagePath = `/images/cases/${slug}.png`

  const body = `${representativeBlock}# ${h1Title}

<figure>
  <img
    src="${imagePath}"
    alt="${caseDisplayName} 피해 회복을 위한 법률 정보 이미지"
  />
...
${buildRoleBody(cleanCaseName, slug)}`

  const full = `---\n${frontmatter}\n---\n${body}`

  return { slug, frontmatter, body, full }
}
